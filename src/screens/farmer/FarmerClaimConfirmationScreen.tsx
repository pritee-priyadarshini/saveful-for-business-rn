...
export function FarmerClaimConfirmationScreen({ route, navigation }: any) {
  ...
  const totalKg = itemsToShow.reduce(
    (sum: number, i: any) => sum + Number(i.claimedQty ?? 0),
    0,
  );
  ...
  const renderSkeleton = () => (
    <View style={[styles.skeletonWrap, contentColumn]}>
      ...
      <Skeleton width="90%" height={normalize(20)} style={styles.skeletonPad} />
      <Skeleton width="100%" height={normalize(120)} borderRadius={normalize(14)} />
      <Skeleton width="100%" height={normalize(80)} borderRadius={normalize(14)} />
